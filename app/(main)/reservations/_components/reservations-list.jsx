...
const ReservationsList = ({ intialData }) => {
  const {
    loading: cancelling,
    fn: cancelBookingFn,
    error: cancelError,
  ...
  const upcomingBookings = intialData?.data?.filter((booking) =>
    ["PENDING", "CONFIRMED"].includes(booking.status)
  );

  const pastBookings = intialData?.data?.filter((booking) =>
    ["COMPLETED", "CANCELLED", "NO_SHOW"].includes(booking.status)
  );

  if (intialData?.data?.length === 0) {
    return (
