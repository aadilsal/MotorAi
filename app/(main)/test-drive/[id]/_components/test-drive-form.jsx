...
import { bookTestDrive } from "@/actions/test-drive"; // Adjust path as needed
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Show error message
        alert(`Failed to book test drive: ${result.error}`);
      }
    } catch (error) {
          ...
          <CardContent className="p-6">
            ...
            <div className="text-sm">
              <p className="font-medium">
                {dealership?.name || "MotorAI Dealership"}
              </p>
              <p className="text-gray-600 mt-1">
                {dealership?.address || "123 Main St, Your City"}
              </p>
              <p className="text-gray-600 mt-3">
                <span className="font-medium">Phone:</span>{" "}
                {dealership?.phone || "+1 234-567-8901"}
              </p>
              <p className="text-gray-600">
                <span className="font-medium">Email:</span>{" "}
                {dealership?.email || "info@motorai.in"}
              </p>
            </div>
          </CardContent>
        </Card>
